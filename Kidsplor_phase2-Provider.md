...
| Field | Detail |
|---|---|
| Product | Kidsplor Provider Portal |
...
6. [AI Assistant Requirements (Nice to Have)](#6-ai-assistant-requirements-nice-to-have)
7. [Notifications & Alerts](#7-notifications--alerts)
8. [Activity Category Taxonomy (Phase 1)](#8-activity-category-taxonomy-phase-1)
9. [User Stories - Priority Matrix](#9-user-stories--priority-matrix)
10. [Integration Requirements](#10-integration-requirements)
11. [Roles & Permissions (Phase 1)](#11-roles--permissions-phase-1)
12. [Data Model - Key Entities (High Level)](#12-data-model--key-entities-high-level)
13. [Acceptance Criteria - Critical Flows](#13-acceptance-criteria--critical-flows)
14. [Open Questions for Developer Team](#14-open-questions-for-developer-team)
...
- **Self-service first.** Providers should be able to do everything without contacting Kidsplor support.
- **AI-assisted.** An embedded AI assistant guides providers through onboarding and common tasks.
- **Parent money flows directly to the provider** via Stripe Connect. Kidsplor takes a platform fee per transaction.
...
- Provider is presented with a clear explanation of how payments work: parents pay at booking; money goes directly to the provider's bank account; Kidsplor deducts a platform fee per transaction.
- Platform fee displayed prominently: **1.4% + $0.30 per transaction** (Starter Plan).
- Provider is redirected to Stripe Connect's hosted onboarding flow (OAuth). Kidsplor must never collect or store raw banking details directly.
...
- Kidsplor deducts its platform fee automatically at payout via Stripe Connect's application fee mechanism.
- Kidsplor never holds client funds beyond the Stripe payout cycle.
...
- Kidsplor platform fee is not refunded on cancellations initiated by the provider (configurable policy — TBC with legal).
...
- Providers can reply via email; the reply is captured and posted back into the Kidsplor thread (via email parsing / webhook — technical implementation TBD with developer team).
...
- Kidsplor uses **Stripe Connect (Standard or Express)** to connect provider bank accounts.
- All raw banking data is handled exclusively by Stripe. Kidsplor stores only the Stripe `account_id`.
...
- Phase 1 Starter Plan: **1.4% + $0.30 per transaction** (Kidsplor application fee).
...
- Configured via the Stripe Connect API; displayed in the Kidsplor UI.
...
| NFR-07 | Payment security              | PCI-DSS compliant via Stripe; no raw card data handled by Kidsplor |
...
- An AI assistant widget ("Kidsplor Assistant") is accessible from all pages via a floating action button (✨).
...
| OQ-06 | Refund policy: does Kidsplor retain the platform fee on a provider-initiated refund? Requires legal sign-off. | Legal / Product | — |
...
| # | Term | Definition |
|---|---|---|
| 01 | Provider | A business or individual offering kids' activities on Kidsplor |
...
| 08 | Platform Fee | Kidsplor's charge per transaction (1.4% + $0.30, Starter Plan) |
...
*End of Document — Kidsplor Provider Portal BRD v1.0*
